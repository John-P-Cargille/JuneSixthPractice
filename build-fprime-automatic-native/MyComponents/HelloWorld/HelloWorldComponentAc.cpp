...
namespace MyComponents {
    ...
    )
  {
    ...
    // Serialize for IPC
    _status = msg.serialize(static_cast<NATIVE_INT_TYPE>(CMD_SAY_HELLO));
    FW_ASSERT (
  ...
  void HelloWorldComponentBase ::
    tlmWrite_GreetingCount(U32 arg, Fw::Time _tlmTime)
  {
    if (this->m_tlmOut_OutputPort[0].isConnected()) {
      ...
      _id = this->getIdBase() + CHANNELID_GREETINGCOUNT;
  ...
  void HelloWorldComponentBase ::
    log_ACTIVITY_HI_Hello(
        const Fw::LogStringArg& greeting
    )
  {
    ...
    _id = this->getIdBase() + EVENTID_HELLO;
    ...
    if (this->m_logOut_OutputPort[0].isConnected()) {
      ...
      _status = greeting.serialize(_logBuff, 20);
      FW_ASSERT(
          _status == Fw::FW_SERIALIZE_OK,
          static_cast<FwAssertArgType>(_status)
      );
    ...
    doDispatch()
  {
    ...
    switch (msgType) {
      ...
      case CMD_SAY_HELLO: {
        ...
        // Deserialize argument greeting
        Fw::CmdStringArg greeting;
        deserStatus = args.deserialize(greeting);
